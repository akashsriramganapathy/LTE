...
import com.github.libretube.test.obj.FreeTubeImportPlaylist
import com.github.libretube.test.obj.FreeTubeVideo
import com.github.libretube.test.obj.FreetubeSubscription
import com.github.libretube.test.obj.FreetubeSubscriptions
...
object ImportHelper {
    ...
    data class LocalImportPlaylist(
        var name: String = "",
    ...
    suspend fun importPlaylists(context: Context, uri: Uri, importFormat: ImportFormat) {
        ...
        when (importFormat) {
            ImportFormat.YOUTUBE_COMPATIBLE -> {
             ...
             ImportFormat.FREETUBE -> {
                ...
                val playlists = playlistFile.orEmpty().map { playlist ->
                    ...
                    // convert FreeTube playlists to local playlists
                    LocalImportPlaylist(
                        playlist.name,
                        playlist.videos.map { it.videoId }
                    )
            ...
            ImportFormat.YOUTUBECSV -> {
                val playlist = LocalImportPlaylist()
                context.contentResolver.openInputStream(uri)?.use { inputStream ->
