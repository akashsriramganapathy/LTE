...
import com.github.libretube.test.LibreTubeApp.Companion.PLAYLIST_DOWNLOAD_ENQUEUE_CHANNEL_NAME
...
import com.github.libretube.test.extensions.serializableExtra
...
class PlaylistDownloadEnqueueService : LifecycleService() {
    ...
    private suspend fun enqueuePublicPlaylist() {
        ...
        amountOfVideos = playlist.videos
        com.github.libretube.logger.FileLogger.i("DownloadManager", "Found $amountOfVideos videos in playlist (Metadata count). Stream count: ${playlist.relatedStreams.size}")
        enqueueStreams(playlistId, playlist.relatedStreams)
        ...
        while (nextPage != null) {
            var playlistPage : com.github.libretube.test.api.obj.Playlist? = null
            var attempt = 0
            while (attempt < 3 && playlistPage == null) {
                try {
                    playlistPage = MediaServiceRepository.instance.getPlaylistNextPage(playlistId, nextPage!!)
                } catch (e: Exception) {
                    attempt++
                    if (attempt >= 3) {
                         com.github.libretube.logger.FileLogger.e("DownloadManager", "Failed to fetch next page for playlist $playlistId", e)
            ...
            if (playlistPage == null) {
                 // Should not happen due to return above, but for safety
                 break
            }
            val validPage = playlistPage!!
            com.github.libretube.logger.FileLogger.d("DownloadManager", "Fetched page with ${validPage.relatedStreams.size} items")

            enqueueStreams(playlistId, validPage.relatedStreams)
            nextPage = validPage.nextpage
        }
    }
    ...
    private suspend fun processSingleStream(stream: StreamItem, playlistId: String) {
        try {
            ...
            if (!DatabaseHolder.Database.downloadDao().exists(videoId)) {
                ...
                // Retry Mechanism for Metadata Fetching
                var videoInfo: com.github.libretube.test.api.obj.Streams? = null
                var attempt = 0
                while (attempt < 3 && videoInfo == null) {
                    try {
                        videoInfo = MediaServiceRepository.instance.getStreams(videoId)
                    } catch (e: Exception) {
                        attempt++
                        if (attempt >= 3) {
                             com.github.libretube.logger.FileLogger.e("DownloadManager", "Failed to enlist video $videoId after 3 attempts", e)
                        } else {
                             delay(1000L * attempt) // Exponential backoff: 1s, 2s
                        }
                    }
                }
                
                if (videoInfo == null) return
                val validVideoInfo = videoInfo!!

                val videoStream = getStream(validVideoInfo.videoStreams, maxVideoQuality)
                val audioStream = getStream(validVideoInfo.audioStreams, maxAudioQuality)
                ...
                val downloadData = DownloadData(
                    ...
                    audioLanguage = audioLanguage.takeIf {
                        validVideoInfo.audioStreams.any { it.audioTrackLocale == audioLanguage }
                    },
                    subtitleCode = captionLanguage.takeIf {
                         validVideoInfo.subtitles.any { it.code == captionLanguage }
                    }
                )
                ...
                com.github.libretube.logger.FileLogger.d("DownloadManager", "Skipping duplicate video: $videoId")
            }
        } catch (e: Exception) {
            e.printStackTrace()
    ...
    private fun getStream(streams: List<StreamFormat>, maxQuality: Int?): StreamFormat? {
