...
import com.github.libretube.test.databinding.DialogImportExportFormatChooserBinding
...
class BackupRestoreSettings : BasePreferenceFragment() {
    ...
    private val createDbFile = registerForActivityResult(CreateDocument(FILETYPE_ANY)) { uri ->
        if (uri == null) return@registerForActivityResult
        lifecycleScope.launch(Dispatchers.IO) {
            BackupHelper.backupDatabase(requireContext().applicationContext, uri)
        }
    }

    private val getDbFile = registerForActivityResult(ActivityResultContracts.GetContent()) { uri ->
        if (uri == null) return@registerForActivityResult
        lifecycleScope.launch(Dispatchers.IO) {
            BackupHelper.restoreDatabase(requireContext().applicationContext, uri)
            withContext(Dispatchers.Main) {
                runCatching {
                    RequireRestartDialog().show(childFragmentManager, this::class.java.name)
                }
    ...
    override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
        ...
        val restoreAdvancedBackup = findPreference<Preference>("restore_legacy")
        restoreAdvancedBackup?.setOnPreferenceClickListener {
            getBackupFile.launch("*/*") // Changed to accept both JSON and DB
            true
        }
    ...
    private fun setupAutoBackupPreferences() {
        ...
        backupTime?.setOnPreferenceClickListener {
            ...
            android.app.TimePickerDialog(requireContext(), { _, selectedHour, selectedMinute ->
                ...
                if (PreferenceHelper.getBoolean(PreferenceKeys.AUTO_BACKUP_ENABLED, false)) {
                     com.github.libretube.test.workers.AutoBackupWorker.enqueueWork(requireContext().applicationContext, ExistingPeriodicWorkPolicy.CANCEL_AND_REENQUEUE)
                }
        ...
        runBackupNow?.setOnPreferenceClickListener {
            ...
            if (path.isEmpty()) {
                 ...
                 com.github.libretube.test.workers.AutoBackupWorker.runNow(requireContext().applicationContext)
            }
            true
        }
    }
