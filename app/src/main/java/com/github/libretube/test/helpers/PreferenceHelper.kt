...
import com.github.libretube.test.enums.SbSkipOptions
import com.github.libretube.test.helpers.LocaleHelper.getDetectedCountry
...
object PreferenceHelper {
    ...
    private val MIGRATIONS = arrayOf(
        ...
        PreferenceMigration(2, 3) {
            // git log -p -- app/src/main/java/com.github.libretube.test/constants/PreferenceKeys.kt | rg "^-.*const val [A-Z_]+" | awk '{printf("%s,", $6);}' | sort | uniq
            listOf(
                "player_audio_format",
                "lbry_hls",
        ...
        PreferenceMigration(5, 6) {
            listOf("dearrow", "dearrow_contribute_key").map { remove(it) }
        },
    )
    ...
    fun clearPreferences() {
        dataStore.clear()
        getDefaultSharedPreferences(LibreTubeApp.instance).edit().clear().apply()
    }
    ...
    fun getDeArrowUserID(): String {
        var uuid = getString(PreferenceKeys.DEARROW_USER_ID, "")
        if (uuid.isEmpty()) {
            // generate a new user id to use for submitting DeArrow data
            uuid = (0 until 30).map { USER_ID_CHARS.random() }.joinToString("")
            putString(PreferenceKeys.DEARROW_USER_ID, uuid)
        }
        return uuid
    }
