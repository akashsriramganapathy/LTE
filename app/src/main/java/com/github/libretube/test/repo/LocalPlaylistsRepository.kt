...
import com.github.libretube.test.api.PlaylistsHelper.MAX_CONCURRENT_IMPORT_CALLS
...
import com.github.libretube.test.extensions.parallelMap
...
class LocalPlaylistsRepository: PlaylistRepository {
        ...
        return DatabaseHolder.Database.localPlaylistsDao().getAll()
            .map {
                Playlists(
                    ...
                    orderIndex = it.playlist.orderIndex
                )
            }
    }
    ...
    override suspend fun importPlaylists(playlists: List<ImportHelper.LocalImportPlaylist>) {
    ...
    override suspend fun updatePlaylistVideos(playlistId: String, videos: List<StreamItem>): Boolean {
        ...
        for (video in videos) {
            ...
            // Should we fetch thumbnail? Maybe not blocking here, or reuse logic if needed
            // Assuming stream items from adapter are good enough
    ...
    override suspend fun updateLocalPlaylistOrder(playlists: List<String>) {
        playlists.forEachIndexed { index, id ->
            if (id.all { it.isDigit() }) {
               DatabaseHolder.Database.localPlaylistsDao().updateOrder(id.toInt(), index) 
            }
        }
    }
}
